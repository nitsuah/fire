...
function parseFidelityPositions(rows) {
    ...
    if (idxSymbol === -1 || idxCurrentValue === -1) return { positions: [], count: 0 };
...
function getAggregateCash(importedPositions, customAccounts) {
    ...
    (customAccounts || []).forEach(acc => {
        if (acc.type === 'Cash' || acc.type === 'Savings') sum += acc.value || 0;
    });
    return sum;
}
...
function getAggregateEquities(importedPositions, customAccounts) {
    ...
    (customAccounts || []).forEach(acc => {
        if (acc.type === 'Brokerage' || acc.type === 'Crypto') sum += acc.value || 0;
    });
    return sum;
}
...
function getAggregateRealEstate(realEstate) {
    return (realEstate || []).reduce((sum, re) => sum + Math.max(0, (re.marketValue || 0) - (re.mortgageBalance || 0)), 0);
}

function getAggregateVehicles(vehicles) {
    return (vehicles || []).reduce((sum, v) => sum + Math.max(0, (v.currentValue || 0) - (v.loanBalance || 0)), 0);
}

function getAggregateNetWorth(state) {
    return getAggregateCash(state.importedPositions, state.customAccounts)
...
function buildProjectionData(state, scenarioOffset) {
    ...
    const annualExpenses = getAnnualExpensesTotal(expenses, insurances, taxRate);
    ...
    const nominalReturn = ((state.projectionSettings.expectedReturn || 0) + offset) / 100;
    ...
    const bullData = [], bearData = [], benchData = [];
...
function calculateEbayFees(price, shippingCharged, categoryRate, adRate) {
    ...
    const standardFee = (totalTransactionVal * ((categoryRate || 0) / 100)) + 0.30;
    const adFee = totalTransactionVal * ((adRate || 0) / 100);
    return standardFee + adFee;
}

function calculateEbayNetProfit(price, cost, shippingCharged, shippingActual, categoryRate, adRate) {
    const gross = (price || 0) + (shippingCharged || 0);
    const fees = calculateEbayFees(price, shippingCharged, categoryRate, adRate);
